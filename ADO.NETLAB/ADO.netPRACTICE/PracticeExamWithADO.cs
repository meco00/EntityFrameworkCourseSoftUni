...
{
    public class PracticeExamWithADO
    {
        private SqlConnection connection;
        private SqlCommand cmd;

        public PracticeExamWithADO(string path ="Bitbucket")
        {
            connection = new SqlConnection($"Server=.;Integrated Security=true");
            CreateOrFindDatabase(path);
            


        }

        private void CreateOrFindDatabase(string path)
        {
            using (connection)
            {
                connection.Open();

            string dbInput = $"CREATE DATABASE {path}";
            cmd = new SqlCommand(dbInput, connection);
            try
            {
            ...
            {
                Console.WriteLine("Database Already Exist. Details: ");
            }
            ...
            connection = new SqlConnection($"Server=.;Database={path};Integrated Security=true");
        }
        ...
        private void UseSearchForFiles()
        {
            Console.WriteLine("Enter extension to search for theese files : ");
            string extension =Console.ReadLine();

            cmd = new SqlCommand(@"EXEC usp_SearchForFiles @extension", connection);
            cmd.Parameters.AddWithValue("@extension", extension);

            var reader = cmd.ExecuteReader();
            Console.WriteLine($"Count of Files {reader.FieldCount}");
            while (reader.Read())
            {
                string name = (string)reader["Name"];
                string size = (string)reader["Size"];

                Console.WriteLine($"File {name} with Size : {size}");
        ...
        private void SearchForUserCommits()
        {
            Console.WriteLine("Enter username to search for user's commits : ");
            string user = Console.ReadLine();
            Console.WriteLine($"{user}  has {UseAllUserCommitsFunction(user)} commits");
        }

        private int UseAllUserCommitsFunction(string name)
        {

            cmd = new SqlCommand(@"SELECT dbo.udf_AllUserCommits(@username)", connection);
        ...
        private void AverageSize()
        {
            
            cmd = new SqlCommand(@"SELECT u.Username AS Username,AVG(f.Size) AS Average 
                                        FROM Users AS u
                                        JOIN Commits AS c ON c.ContributorId=u.Id
                                        JOIN Files AS f ON f.CommitId=c.Id
                                        GROUP BY u.Username
                                        ORDER BY Average DESC,u.Username ASC", connection);

            var reader = cmd.ExecuteReader();
            Console.WriteLine("(10)AverageSize");
            Console.WriteLine($"Count Of Rows : {reader.FieldCount}");
            while (reader.Read())
            {
                string name = (string)reader["Username"];
                decimal size = (decimal)reader["Average"];

               
                Console.WriteLine($"{name} : AverageSize: {size}");
        ...
        private void FrontEnd()
        {
            ...
            cmd = new SqlCommand(@"SELECT Id,Name,Size FROM Files
                                      WHERE Size >1000 AND NAME LIKE '%html%'
                                      ORDER BY Size DESC,Id ASC ,Name ASC", connection);

            var reader = cmd.ExecuteReader();

            Console.WriteLine("(6)Front-End");

            while (reader.Read())
            {
                string name = (string)reader["Name"];
                decimal size = (decimal)reader["Size"];
        ...
        private void Initialize()
        {
                 ...
                 cmd = DataDefinitionLanguage(connection);

                cmd.ExecuteNonQuery();


                cmd = DataSet(connection);

                cmd.ExecuteNonQuery();

                cmd = AllUserCommitsFunction();

                cmd.ExecuteNonQuery();

                cmd = SearchForFilesProcedure();
        ...
        private SqlCommand SearchForFilesProcedure()
        {
            return new SqlCommand(@"CREATE PROCEDURE usp_SearchForFiles(@fileExtension VARCHAR(MAX))
                                        AS
                                        
                                        SELECT Id,Name,CAST(Size as varchar)+'KB' AS Size
                                        FROM Files
                                        WHERE Name Like '%'+@fileExtension
                                        ORDER BY Id ASC,Name ASC,Size DESC", connection);
        }

        private SqlCommand AllUserCommitsFunction()
        {
            return new SqlCommand(@"CREATE FUNCTION udf_AllUserCommits(@username VARCHAR(MAX))
                                    RETURNS INT
                                    AS
                                    BEGIN
        ...
        private SqlCommand DataSet(SqlConnection connection)
        {
        ...
        private SqlCommand DataDefinitionLanguage(SqlConnection connection)
        {
            return new SqlCommand(
                              @$"CREATE TABLE Users
                    (
